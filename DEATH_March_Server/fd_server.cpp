	 ...
	 */
		...
		if ((ms = accept(s1, (struct sockaddr *) &client, &len)) < 0)
		{
			...
			for (;;)
			{
				...
				vector<char> data(binary,binary +size);
				...
				Mat detectFaceImage = detectFaceInImage(image, filename);
						     ...
						     //strcpy(o_buf,buf);
						     //strcat(o_buf,"_result.jpg");
			    
						     //imwrite(o_buf,detectFaceImage);
				
						     vector<uchar> outbuf;
						     vector<int>param=vector<int>(2);
						     param[0]=CV_IMWRITE_JPEG_QUALITY;
				param[1]=40;//default(95) 0~9 圧縮レベル 9は最高圧縮
 
				imencode(".jpg",detectFaceImage,outbuf,param);//３は品質デフォが3
				...
				if ((ds = accept(s2, (struct sockaddr *) &client, &len)) < 0)
				{
					...
					exit(1);
						     ...
						     //size=filesize(getFileName(o_buf));
